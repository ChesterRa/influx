#!/usr/bin/env python3
"""
Remove records with empty IDs from dataset
"""
...
backup_file = Path("data/latest/latest_backup_empty_ids.jsonl")
    ...
    for line_num, line in enumerate(infile, 1):
        ...
        try:
            ...
            if not data.get('id') or data.get('id') == '':
                ...
                print(f"Removing line {line_num}: empty ID for handle @{handle}")
                continue
...
if 'notes' not in manifest:
    manifest['notes'] = []
manifest['notes'].append(f"Empty ID removal: {removed_count} records with empty IDs removed")
